...
export default function ClassroomProjector({
  ...
  const [bubbles, setBubbles] = useState<{ id: string; name: string; roll: string; key: number; x: number; y: number }[]>([]);
  ...
  useEffect(() => {
    ...
    socket.on('attendance:new', (entry: AttendanceEntry) => {
      ...
      // QR is roughly 512px, safe zone is 600x600 in center
      // We pick a random edge and random position along it
      const side = Math.floor(Math.random() * 4); // 0=top, 1=right, 2=bottom, 3=left
      const spread = 80; // random offset percentage
      let x = 50, y = 50;
      
      if (side === 0) { y = 10; x = 10 + Math.random() * spread; }
      else if (side === 1) { x = 90; y = 10 + Math.random() * spread; }
      else if (side === 2) { y = 90; x = 10 + Math.random() * spread; }
      else { x = 10; y = 10 + Math.random() * spread; }

      setBubbles((prev) => [...prev, { id: entry.student_enrollment || String(key), name: entry.student_name, roll: entry.student_enrollment, key, x, y }]);
              ...
              <div className="absolute inset-[-400px] pointer-events-none overflow-hidden z-0">
                <AnimatePresence>
                  {bubbles.map((bubble) => (
                    <motion.div
                      key={bubble.key}
                      initial={{ opacity: 0, scale: 0, y: 50, x: 0 }}
                      animate={{ opacity: 1, scale: 1, y: 0, x: (Math.random() - 0.5) * 40 }}
                    ...
                    >
                      ...
                      <div className="flex flex-col whitespace-nowrap">
                        <span className="text-sm font-bold text-white">{bubble.name}</span>
                        <span className="text-xs text-cyan-400">{bubble.roll}</span>
                      </div>
                    </motion.div>
                  ))}
                </AnimatePresence>

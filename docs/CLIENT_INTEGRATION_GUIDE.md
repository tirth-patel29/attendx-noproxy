...
The projector may render the flash in **isoluminant chroma** (the dual-state projector mechanism) so that a 4:2:0 video stream crushes the boundaries. Your local camera decodes it via raw chroma; your scanner logic is unchanged — just decode normally and treat a token ≥ 1 char as valid flash.
...
- **Projector isoluminance spec:** `docs/README.md (dual-state projector)`

 ...
 *
 * Mints base62 tokens aligned to absolute Unix millisecond boundaries
 * (epoch = floor(now / intervalMs)).
 *
 * Features:
...
export class MetronomeService {
  ...
  /**
   * Broadcast real-time attendance update to a session room.
   */
  broadcastAttendance(sessionUuid: string, entry: any): void {
    if (this.io) {
      this.io.to(`session:${sessionUuid}`).emit('attendance:new', entry);
    }
  }
  ...
  async startSession(sessionUuid: string): Promise<void> {
    ...
    await this.mintTokens(
      sessionUuid,
      config.metronome.lookaheadTokens + 1
    );

    // Set up interval aligned to metronome period
    const interval = setInterval(async () => {
      try {
        await this.mintTokens(sessionUuid, 2);
      } catch (err) {
  ...
  private async mintTokens(
    ...
    for (let i = 0; i < count; i++) {
      ...
      const expiresAt =
        createdAt + config.judge.tokenValidityWindowMs;

      // Check if we already have this epoch in memory to avoid redundant work
      const existingInCache = tokenCache
        .getRecentTokens(sessionUuid)
      ...
      const tokenVal = generateToken(
        config.metronome.tokenLength,
        config.metronome.tokenCharset
      );
  ...
  /**
   * Periodic DB cleanup of old expired tokens
   * (called every 60s, not every 3s)
   */
  private async purgeExpiredDbTokens(): Promise<void> {
    const threshold =
      Date.now() -
      (config.judge.tokenValidityWindowMs + 30000);
  ...
  /**
   * Get current active tokens for a session
   * (for debugging/monitoring/tests)
   */
  ...
  /**
   * Look up a token by (session, value) REGARDLESS of current expiry,
   * cache-first with DB fallback.
   *
   * Used by the Layer-3 judge to test whether the token was LIVE at
   * the claimed instant (membership), not just whether it is still
   * live now.
   */
  async findToken(
    sessionUuid: string,
    ...
    const res = await query<TokenRecord>(
      `SELECT *
       FROM active_tokens
       WHERE session_uuid = $1 AND token_val = $2`,
  ...
  /**
   * Verify a token is live using the in-memory cache first,
   * falling back to Postgres only on a cache miss.
   */
  ...
  /**
   * Verify a token is live (exists + not yet expired)
  ...
  /**
   * Verify a token and that it is NOT expired
